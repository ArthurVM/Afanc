class Tree(object):
    ...
    def makeJsonLine(self, dbdict):
        ...
        if self.probability == None:
            ...
            json_data = { "name" : self.name, "reads" : self.taxa_reads, "percentage" : self.clade_perc, "score" : self.probability, "taxon_id" : self.ncbi_taxID, "accession" : acc }
    ...
    def find_local_max(self, local_threshold, return_all_nodes=False):
        ...
        tips = []
        branch_mapped_reads = sum([c.taxa_reads for c in self.traverse()][:-1])
        ...
        for c in self.traverse():
            ...
            ## calculate the local threshold based on the number of reads which map to the mother clade
            weighted_threshold = self.clade_perc * (local_threshold/100)

            # print(c.name, c.level_int, weighted_threshold)

            if c.clade_perc >= weighted_threshold:
                tips.append(c)
