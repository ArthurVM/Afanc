...
def redistribute_lca_hierarchical(
    ...
    ## preserve raw kraken2 counts for scoring
    root = root_node.root()
    ...
    for family in family_roots:
        ...
        for donor in donors:
            ## remove ambiguous direct reads before redistribution
        ...
        ## refresh clade totals after redistribution
...
def collect_direct_donors(family, scoring_set, min_reads):
    ...
    for node in family.traverse():
        ...
        if node.raw_taxon_reads <= 0 or node.raw_taxon_reads < min_reads:
            continue
        donors.append(node)
    ## process child donors before parents
...
def trickle_down(
    ...
    if not allocations:
        ## leave reads unassigned when no branch has a positive weight
        routes.append(
            {
                "target": current.name,
                "target_taxid": current.ncbi_taxID,
...
def weight_branches(
    ...
    for branch in branches:
        ...
        if metric is None:
            ## use read evidence when mash distances are unavailable
