...
def prepare_variant_reference(
    ...
    ## TODO skip rewriting an unchanged source reference
...
def write_depth_bed(
    ...
    ## output is a depth table despite the .bed suffix
...
def sample_alleles_from_vcf(
    ...
    with open_maybe_gzip(vcf_path) as fin:
        for line in fin:
            ...
            ## lineage models currently support SNPs only
            ...
            if len(parts) < 10:
                ## treat every simple ALT as present without genotype columns
                for alt in alts:
...
def passes_filters(
    ...
    if min_dp is not None:
        ## TODO use FORMAT/DP for multi-sample VCFs
