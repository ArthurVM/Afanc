...
def runTest(args):
    ...
    print(f"Sample ID\tTruth Value\tResults\tCluster classification", file=tsv_handle)
    ...
    for fq1, fq2, truth_val in samples_box:
        ...
        if nomap:
            runline = f"afanc screen -o {prefix} -n 10 {args.db} {fq1} {fq2} -a > {prefix}.log"
              
        else:
            runline = f"afanc screen -o {prefix} -n 10 {args.db} {fq1} {fq2} -c > {prefix}.log"

        if not os.path.exists(prefix):
            command(runline, "AFANC-TEST").run_comm(0)
        ...
        print(f"{prefix} :: TRUTH={truth_val}\tCLUSTER_RESULTS={cluster_hits}\t{cluster_result}\n")
        print(f"{prefix}\t{truth_val}\t{cluster_hits}\t{cluster_result}", file=tsv_handle)
...
def checkHits(results_json, truth_val):
    ...
    with open(results_json, "r") as fin:
        ...
        clustering_hits = [record["clustering"] for record in hits.values() if record.get("clustering")]

        cluster_results, cluster_hits = getClusterHits(clustering_hits, truth_val)

        results = { "cluster" : [cluster_results, cluster_hits] }
