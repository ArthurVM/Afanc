...
def fastANI(prefix, fastas):
    ...
    print(argline)
    command(argline, "FASTANI").run_comm(0, None, None)


    return outfile

if __name__=="__main__":
    fasta_dir="/home/amorris/BioInf/M_spongiae/Myco_ANI_DB"
    out_dir="/home/amorris/BioInf/M_spongiae/"
    prefix="sponge"
    key_dict="/home/amorris/BioInf/M_spongiae/Dataset_1_species.csv"
    ...
    # dist_out = mash(prefix, fasta_list)
    dist_out = fastANI(prefix, fasta_list)

    distance_matrix = readDistOut(dist_out, reversed_dict)

    distance_matrix.to_csv(f"{prefix}.dist")
